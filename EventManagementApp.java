...
class DBConnection {
   private static final String URL = "jdbc:mysql://localhost:3306/eventdb";
    ...
    private static final String PASS = "Lakshya@18";
...
class CustomerDAO {
    ...
    public int saveEvent(int custId, String type, String date, String time) throws Exception {

    String sql = "INSERT INTO events (customer_id, event_type, event_date, event_time) VALUES (?,?,?,?)";

    Connection con = DBConnection.getConnection();
    if (con == null) {
        throw new Exception("JDBC Not Connected");
    }
    ...
    try (PreparedStatement ps = con.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
        ...
        java.util.Date t = new SimpleDateFormat("hh:mm a").parse(time);
...
public void savePayment(int eventId, double amount) throws Exception {

    String sql = "INSERT INTO payments (event_id, amount) VALUES (?, ?)";
...
class EngagementEvent extends Event {
    ...
    public String extraDetails() { return "Couple: " + coupleNames + "\nGuests: " + guestCount; }
}
...
class MarriageForm extends EventForm {
    ...
    new PaymentPage(eventId, ev, prevPage).setVisible(true);
    dispose();
    ...
    JOptionPane.showMessageDialog(
        this,
        "❌ Event could not be saved in database.",
        "DB Error",
...
class PaymentPage extends JFrame {
    ...
    public PaymentPage(int eventId, Event ev, EventSelection prevPage) {
        ...
        payNow.addActionListener(e -> {
            ...
            try {
                ...
                new CustomerDAO().savePayment(eventId, amount);
